...
Solr will guarantee that a node having one or more replicas of the collection being created will always host a replica of the `withCollection`. If enough replicas of the `withCollection` do not already exist then new replicas of `NRT` type will be automatically created.
This invariant will be maintained during any future collection operations such as adding replicas, moving replicas or splitting shards either via API or by Solr's Autoscaling feature.
